...
// By Module Variables
var param = {
  url: "https://webreg.vghks.gov.tw/wps/portal/web/realtimequery",
  fn_log: "vghks.json",
...
casper.start().open(param.url).then(function() {
  ...
  this.then(function(){
    ...
    for(var i=0;i<$("[href='#']").length;i++){
      var dep=$("[href='#']").eq(i).text().replace(/\s/g,"");
      parse(i,dep)();
    }
  ...
  return (function() {
    casper.thenOpen(param.url).then(function() {
      ...
      this.then(function(){
        ...
        for (var j = 0; j < $(".txt_r").length; j++) {
          var doc_name = $(".clinic_tb").find("tr").eq(j+1).find("td").eq(3).text().replace(/\s/g,"");
          var _ampm = $(".clinic_tb").find("tr").eq(j+1).find("td").eq(1).text();
          var _p_num = $(".txt_r").eq(j).text().replace(/\D+/g, "");
          if(debug){
            this.echo(dep);
            this.echo(doc_name);
          }
          if((doc_name=="")||(doc_name=="--")||(doc_name=="1")||(_ampm=="")||(dep=="")||(dep=="X光檢查")||(_p_num=="")){
