// Casper Initialization ====================================================
// OPD Tracking Module
// By balance wu
// Last Modified: 2018/08/08
// One-time Parsing: no
// Actual Patient Number: no(now number)
// Note:
//12:00_14:00 cant parse the data will crash!
var casper = require('casper').create({
  verbose: true,
  clientScripts: ['lib/jquery-1.11.1.min.js'],
  pageSettings: {
    webSecurityEnabled: false,
    loadImages: false,
    loadPlugins: false
  },
  waitTimeout: 30000
  //logLevel: 'debug'
});
var $ = require('lib/jQasper.js').create(casper);
var fs = require('fs');

// By Module Variables
var param = {
  url: "https://ws.eck.org.tw/EckNetReg/KQueryStatus/QueryStatus01.aspx",
  fn_log: "eck.json",
  post_url: "http://localhost/opdtrack/engine_debug.php",
  hosname: "行天宮醫療志業醫療財團法人恩主公醫院",
  module: "eck",

};

var opd_time = {
  0: '上午',
  1: '下午',
  2: '晚上'
}

var log = {
  date:"",
  datetime:"",
  records:[],
};

var logAll = [];
var debug=true;
// MAIN =====================================================================
casper.on("remote.message", function(message) {
  //this.echo("\tremote console.log: " + message);
});

casper.start().open(param.url).then(function() {
  clear_timeout();
  this.echo("time stop");
  this.echo("Entering " + param.url);
  this.then(function() {
    for(var i=0;i<$(".Blod_RedFont_44px").length;i++){

      parse(i)();
    }

  });
  //-------- Test
  //parse_progress(ampm_dom.eq(1), dep_dom.eq(1));
  //-------
}).then(function() {
  etime = now_time();
  log.date = get_date();
  log.datetime = get_date() + " " + get_now();
  //log_now(etime);
  this.echo("\nPosting to Server...");
  make_doPost(param.post_url, "set_records", log, true)();

}).then(function(){
  this.echo("\nFinalizing...");
  make_doPost(param.post_url, "final", {module: param.module}, true)();
});

function parse(i) {
  return (function() {
    casper.thenOpen(param.url).then(function() {

      this.then(function() {
        $(".Blod_RedFont_44px").eq(i).click();
      }).then(function() {
          var doc_name = $("#lb_DrName").text().replace(/醫師/,"").replace(/代/,"") ;
          var dep = $("#lb_ClinicName").text().replace(/診/,"").replace(/\d+/g,'').replace(/\s/g,"").replace(/【/,"").replace(/】/,"").replace(/NE/,"").replace(/-/,"");
          var _ampm = opd_time[casper.cli.args[0]];
          var _p_num = $('span[title="已看診"]').length;
          if ((doc_name == "")  || (dep == "") || (_p_num == "0")) {

          } 
          else {
            if(debug){
              this.echo(doc_name);
              this.echo(dep);
              this.echo(_p_num);
            }
            log.records.push({
              mod: param.module,
              hos_id: null,
              hos_name: param.hosname,
              opdtime: _ampm,
              hos_grp: null,
              hos_dep: dep,
              name: doc_name,
              note: "",
              num: _p_num
            });
          }});

    });
  })
}

function now_time() {
  return (
    casper.evaluate(function() {
      return (new Date().toJSON().slice(0, 10));
    })
  );
}

function get_date(){
  var now = new Date();
  var y = now.getFullYear()*1;
  var m = now.getMonth()*1+1;
  var d = now.getDate()*1;
  return(conv(y) +"-" + conv(m) + "-" + conv(d));
  
  function conv(i){
    if (i<10){return("0" + i.toString());
    }else{return(i.toString());}
  }
}
function get_now(){
  var now = new Date();
  var h = now.getHours()*1;
  var m = now.getMinutes()*1;
  var s = now.getSeconds()*1;
  return(conv(h) +":" + conv(m) + ":" + conv(s));
  
  function conv(i){
    if (i<10){return("0" + i.toString());
    }else{return(i.toString());}
  }
}

function log_now(_name){
	fs.write("data/" + param.module + "/" + _name + param.fn_log, JSON.stringify(log.records));
}
function make_doPost(url, _act, _data, disp){
  return function(){
    casper.thenOpen(url, {
        method: "post",
        data: {
          "act": _act,
          "data": JSON.stringify(_data)
        }
      }
    ).then(function(){
      if (disp){
      casper.echo(casper.fetchText("body"));
      }
    });
  }
}
function clear_timeout(){
  casper.evaluate(function(){
    var ix = setTimeout(function(){},0);
    for (var i=0;i<=ix;i++){
      clearTimeout(i);
    }
  });
}

casper.run();
